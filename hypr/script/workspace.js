...
async function main() {
  ...
  program2.command("toggle").action(() => {
    const not = monitors.find((monitor) => !monitor.focused);
    Bun.spawn(["hyprctl", "dispatch", "focusmonitor", not.name]);
  });
  program2.command("move").action(() => {
    const not = monitors.find((monitor) => !monitor.focused);
    console.log(not);
    Bun.spawn([
      "hyprctl",
      "dispatch",
      "movetoworkspace",
      not.activeWorkspace.id.toString()
    ]);
  });
  program2.parse();
}
var res = Bun.spawn(["hyprctl", "monitors", "-j"]);
var json = await new Response(res.stdout).text();
var monitors = JSON.parse(json);
await main();
