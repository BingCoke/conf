...
// index.ts
var { stdout } = Bun.spawn(["hyprctl", "activewindow", "-j"]);
var res = await new Response(stdout).text();
var window = JSON.parse(res);
var isFloating = window["floating"];
var program2 = new Command;
program2.version("0.0.1", "-v --version", "look version");
program2.parse(process.argv);
program2.command("float").arguments("<hyprctl>").arguments("<params...>").action((hyprctl, params) => {
  if (isFloating) {
    const { stdout: stdout2 } = Bun.spawn(["hyprctl", "dispatch", hyprctl, ...params]);
  }
});
program2.command("tile").arguments("<hyprctl>").arguments("<params...>").action((hyprctl, params) => {
  if (!isFloating) {
    const { stdout: stdout2 } = Bun.spawn(["hyprctl", "dispatch", hyprctl, ...params]);
  }
});
