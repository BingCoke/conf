...
# 更新状态栏
refresh() {
    _system='';_keyboard=' ';_resource=' ';_cpu=''; _mem=''; _date=''; _vol=''; _bat='';   # 重置所有模块的状态为空
    source $DWM/statusbar/temp                                           # 从 temp 文件中读取模块的状态
    #xsetroot -name "$_icons$_coin$_cpu$_mem$_date$_vol$_bat"             # 更新状态栏
    xsetroot -name "$_system$_keyboard$_resource$_cpu$_mem$_date$_vol$_bat"             # 更新状态栏
}
...
cron() {
    ...
    while true; do update vol resource;      sleep 60;  done &              # 每隔60s更新音量 图标
    while true; do update bat;            sleep 60; done &              # 每隔300s更新电池
...
case $1 in
    ...
    updateall) system update cpu mem date vol bat resource keyboard; refresh ;;
esac
